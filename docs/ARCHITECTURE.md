...
5. The Media Foundation source reads the pipe and prefers RGB32 1280×720 at 30 fps. When the pipe is disconnected, invalidated, or stale, it emits the shared black off frame with a centered red crossed-camera icon. Selectable NV12 720p remains a compatibility fallback for Windows Camera and Zoom; 1080p is excluded without evidence.
...
The dashboard is not part of the output clock. Its visible live previews refresh at 30 fps, convert BGRA directly into one display-sized texture allocation, and reduce wakeups while hidden in the tray. Its output preview uses the same shared off frame as the Media Foundation source whenever automation is stopped.
