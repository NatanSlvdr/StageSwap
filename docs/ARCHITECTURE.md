...
The dashboard is not part of the output clock. Each visible preview has a dedicated latest-only conversion worker: pending frames are replaced rather than queued, bounded 480×270 textures are prepared off the egui thread, and the UI uploads only the newest completed image at its 30 fps cadence. Wakeups remain throttled while hidden in the tray. While automation is stopped, the runtime keeps publishing the same shared off frame used by the Media Foundation source at 30 fps.
