...
11. Enable and disable the optional webcam crop and confirm the same centered crop feeds both the preview and virtual-camera output.
12. Record Task Manager CPU for 60 seconds with the dashboard open and with the app hidden in the tray. Confirm runtime-owned FPS remains populated while hidden. Compare CPU with the previous build; it must improve materially, but there is no fixed percentage gate.
13. Install StageSwap beside Automatic Screen Camera. Confirm both cameras enumerate independently, neither can be selected as StageSwap's physical input, and StageSwap cleanup leaves the other product's files, data, registry state, startup value, and camera registration unchanged.
