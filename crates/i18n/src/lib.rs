...
pub fn text<'a>(locale: Locale, source: &'a str) -> Cow<'a, str> {
    Cow::Borrowed(catalog!(locale, source, {
        ...
        "Restart capture automatically when black or unavailable" => ("Redémarrer automatiquement la capture si elle est noire ou indisponible", "Reiniciar automáticamente la captura si está en negro o no disponible"),
        ...
        "On — Checks the selected screen every 30 seconds and restarts capture after two black or unavailable results. Black content can trigger a restart." => ("Activé — Vérifie l’écran sélectionné toutes les 30 secondes et redémarre la capture après deux résultats noirs ou indisponibles. Un contenu noir peut déclencher un redémarrage.", "Activado: comprueba la pantalla seleccionada cada 30 segundos y reinicia la captura tras dos resultados en negro o no disponibles. El contenido negro puede activar un reinicio."),
